...
export const validateEdges = (pack: Pack, edges: Edge[], text: string) => {
 ...
 return { valid, requiredMet: pack.required.every(has), forbiddenFound: pack.forbidden.some(has), invalidCount: edges.length-valid.length };
};
export const graphDiff = (before: Edge[], after: Edge[]) => { const key=(e:Edge)=>`${e.source}:${e.target}:${e.type}`; return { removed: before.filter((b)=>!after.some((a)=>key(a)===key(b))), added: after.filter((a)=>!before.some((b)=>key(a)===key(b))) }; };
