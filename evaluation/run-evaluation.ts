...
import { confusion, expectedDecision, macroF1, type Label } from "./metrics";
import { mkdirSync, writeFileSync } from "node:fs";

type EvaluationCase = (typeof cases)[number];
type EvaluatedRow = ReturnType<typeof evaluateCase>;

function evaluateCase(testCase: EvaluationCase) {
  ...
  const expected = expectedDecision(testCase.label as Label);
  ...
  return {
    ...
    label: testCase.label as Label,
    raw
  };
}

function markdownTable(rows: EvaluatedRow[]) {
...
const sweep = [] as Array<Record<string, unknown>>;
for (const confidence of [.4, .5, .6, .7, .72, .8, .9]) for (const retrieval of [.1, .2, .4, .58, .7, .85]) {
...
if (selected.hiddenMisconceptionRecall !== 1 || selected.validMethodFalsePositiveRate !== 0 || selected.ambiguityRoutingAccuracy !== 1 || JSON.stringify(selected.confusion) !== JSON.stringify({ tp: 15, fp: 0, tn: 9, fn: 0 })) throw new Error("Configured operating point does not reproduce required canonical targets");
...
const robustnessRows = (robustnessCases as EvaluationCase[]).map(evaluateCase);
